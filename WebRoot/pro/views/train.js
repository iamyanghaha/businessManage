...
function eventInit(){
	...
	$('body').on('click', '#addButton', function(){
		$("#introduction").val("");
		openadd('新增培训')//打开新增对话框
	});
	//新增时保存按钮事件
	$('body').on('click', '#add', function(e){
		var params = {	
				task: $("#course").val(),
				teacher: $("#lecturer").val(),
				date: $("#date").val(),
				introduction: $("#introduction").val()
		}
		AjaxUtil.request({url:path+"/post/train/addTrain",params:params,type:'json',callback:function(json){
			if (json.returnCode == 0){
				...
				//请求数据
				AjaxUtil.request({url : path + "/post/train/selectAllTrain",params : params,type : 'json',callback : function(json) {
					refreshData(json);	
					}
				});
			}else{
	...
	$('body').on('click', '#del', function(e){
		...
		AjaxUtil.request({url:path+"/post/train/deleteTrain",params:params,type:'json',callback:function(json){
			if (json.returnCode == 0){
				$('#cancelDel').click();
				dataInit();
			}else{
	...
	$('body').on('click', '#editId', function(e){
		...
		AjaxUtil.request({url:path+"/post/train/selectSingleTrain",params:params,type:'json',callback:function(json){
			if (json.returnCode == 0){
				$("#course").val(json.bean.task);
				$("#lecturer").val(json.bean.teacher);
				$("#date").val(json.bean.date);
				$("#introduction").val(json.bean.introduction);
			}else{
				alert("数据获回显失败！");
			}
		}
	...
	$('body').on('click', '#edt', function(e){
		...
		AjaxUtil.request({url:path+"/post/train/updateTrain",params:params,type:'json',callback:function(json){
			if (json.returnCode == 0){
				//清除文本编辑框信息
				$("#course").val("");
				$("#lecturer").val("");
